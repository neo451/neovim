...
---                         default to first client of buffer
---@return vim.quickfix.entry[] # See |setqflist()| for the format
function M.links_to_items(links, bufnr, position_encoding)
  bufnr = vim._resolve_bufnr(bufnr)
  if position_encoding == nil then
    vim.notify_once(
  ...
  for _, link in ipairs(links) do
    ...
    if uri and range then
      ...
      if vim.startswith(uri, 'file:///') then
        ...
        col = get_line_byte_from_position(bufnr, range['start'], position_encoding) + 1
        end_lnum = range['end'].line + 1
        end_col = get_line_byte_from_position(bufnr, range['end'], position_encoding) + 1
      end
